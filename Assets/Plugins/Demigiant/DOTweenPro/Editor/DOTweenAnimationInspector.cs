...
namespace DG.DOTweenEditor
{
 ...
 new Dictionary<DOTweenAnimation.AnimationType, Type[]>() {
...
#endif
#if true // TEXTMESHPRO_MARKER
        private static readonly Dictionary<DOTweenAnimation.AnimationType, Type[]> _TMPAnimationTypeToComponent = new()
        {
        ...
        private static readonly string[] _AnimationType =
        {
            "None",
            "Move", "LocalMove",
            "Rotate", "LocalRotate",
...
#pragma warning restore 414
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.Label(_src.animationType + (string.IsNullOrEmpty(_src.id) ? "" : " [" + _src.id + "]"),
            ...
            if (GUILayout.Button("▼", DeGUI.styles.button.toolIco))
            {
                ComponentUtility.MoveComponentDown(_src);
            }
...
//            _src.isValid = Validate(); // Moved down
            ...
            // Choose target
            GUILayout.BeginHorizontal();
            _src.isActive = EditorGUILayout.Toggle(new GUIContent("", "If unchecked, this animation will not be created"), _src.isActive,
                GUILayout.Width(14));
            EditorGUI.BeginChangeCheck();
            EditorGUI.BeginChangeCheck();
...
//                _src.animationType = (DOTweenAnimation.AnimationType)EditorGUILayout.EnumPopup(_src.animationType, EditorGUIUtils.popupButton);
                ...
                _src.animationType =
                    AnimationToDOTweenAnimationType(
                        _AnimationType[EditorGUILayout.Popup(DOTweenAnimationTypeToPopupId(_src.animationType), _AnimationType)]);
                ...
                _src.autoKill = DeGUILayout.ToggleButton(_src.autoKill,
                    new GUIContent("AutoKill", "If selected, the tween will be killed when it completes, and won't be reusable"));
                GUILayout.EndHorizontal();
...
#if true // UI_MARKER
                    if (_src.animationType == DOTweenAnimation.AnimationType.Fade && targetGO.GetComponent<CanvasGroup>() != null &&
                        targetGO.GetComponent<Image>() != null)
                    {
                ...
                if (_chooseTargetMode == ChooseTargetMode.BetweenCanvasGroupAndImage &&
                    _src.forcedTargetType != DOTweenAnimation.TargetType.Unset)
                {
                    var fadeTargetType = (FadeTargetType)Enum.Parse(typeof(FadeTargetType), _src.forcedTargetType.ToString());
                ...
                _src.isSpeedBased = DeGUILayout.ToggleButton(_src.isSpeedBased,
                    new GUIContent("SpeedBased", "If selected, the duration will count as units/degree x second"), DeGUI.styles.button.tool,
                    GUILayout.Width(75));
                GUILayout.EndHorizontal();
                ...
                switch (_src.animationType)
                {
                    ...
                    case DOTweenAnimation.AnimationType.Text:
                        ...
                        _src.optionalString =
                            EditorGUILayout.TextField(
                                new GUIContent("Custom Scramble", "Custom characters to use in case of ScrambleMode.Custom"),
                                _src.optionalString);
                        break;
                    case DOTweenAnimation.AnimationType.PunchPosition:
                    case DOTweenAnimation.AnimationType.PunchRotation:
                    case DOTweenAnimation.AnimationType.PunchScale:
                        GUIEndValueV3(targetGO);
                        canBeRelative = false;
                        _src.optionalInt0 = EditorGUILayout.IntSlider(new GUIContent("    Vibrato", "How much will the punch vibrate"),
                            _src.optionalInt0, 1, 50);
                        _src.optionalFloat0 =
                            EditorGUILayout.Slider(
                                new GUIContent("    Elasticity",
                                    "How much the vector will go beyond the starting position when bouncing backwards"), _src.optionalFloat0,
                                0, 1);
                    ...
                    case DOTweenAnimation.AnimationType.ShakeScale:
                        GUIEndValueV3(targetGO);
                        canBeRelative = false;
                        _src.optionalInt0 = EditorGUILayout.IntSlider(new GUIContent("    Vibrato", "How much will the shake vibrate"),
                            _src.optionalInt0, 1, 50);
                        using (new GUILayout.HorizontalScope())
                        {
                            _src.optionalFloat0 = EditorGUILayout.Slider(new GUIContent("    Randomness", "The shake randomness"),
                                _src.optionalFloat0, 0, 90);
                            _src.optionalShakeRandomnessMode =
                                (ShakeRandomnessMode)EditorGUILayout.EnumPopup(_src.optionalShakeRandomnessMode, GUILayout.Width(70));
                        }
                        _src.optionalBool1 =
        ...
        private bool Validate(GameObject targetGO)
        {
            ...
            {
                foreach (var t in _TMPAnimationTypeToComponent[_src.animationType])
                {
                    srcTarget = targetGO.GetComponent(t);
                    if (srcTarget != null)
                    {
        ...
        private void GUIEndValueV3(GameObject targetGO, bool optionalTransform = false)
        {
            ...
            {
                EditorGUILayout.HelpBox("NOTE: when using a UI target, the tween will be created during Start instead of Awake",
                    MessageType.Info);
            }
#endif
        }
