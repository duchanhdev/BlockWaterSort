...
namespace DG.Tweening
{
    ...
    {
        /// <summary>
        ///     Applies the skew as-is (like normal skew works): the longer the text-span the higher the last character will
        ///     be
        /// </summary>
        Default,
    ...
    public static class ShortcutExtensionsTMPText
    {
        ...
        public static TweenerCore<Color, Color, ColorOptions> DOGlowColor(this TMP_Text target, Color endValue, float duration,
            bool useSharedMaterial = false)
        {
        ...
        /// </param>
        public static TweenerCore<string, string, StringOptions> DOText(this TMP_Text target, string endValue, float duration,
            bool richTextEnabled = true, ScrambleMode scrambleMode = ScrambleMode.None, string scrambleChars = null)
        {
    ...
    {
        private static readonly Dictionary<TMP_Text, DOTweenTMPAnimator> _targetToAnimator = new();

        private readonly List<CharTransform> _charTransforms = new();
        private TMP_MeshInfo[] _cachedMeshInfos;
        private bool _ignoreTextChangedEvent;

        /// <summary>
        ///     Creates a new instance of the <see cref="DOTweenTMPAnimator" />, which is necessary to animate
        ///     <see cref="TMP_Text" /> by single characters.
        ///     <para />
        ///     If a <see cref="DOTweenTMPAnimator" /> already exists for the same <see cref="TMP_Text" /> object it will be
        ///     disposed
        ///     (but not its tweens, those you will have to kill manually).
        ///     If you want to animate the whole text object you don't need this, and you can use direct <see cref="TMP_Text" /> DO
        ///     shortcuts instead.
        ///     <para />
        ///     IMPORTANT: the <see cref="TMP_Text" /> target must have been enabled/activated at least once before you can use it
        ///     with this
        /// </summary>
        ...
        public void Refresh()
        {
            ...
            for (var i = totCurrent; i < totChars; ++i)
            {
        ...
        private struct CharTransform
        {
            ...
            public void UpdateGeometry(TMP_Text target, Vector3 offset, Quaternion rotation, Vector3 scale, TMP_MeshInfo[] cachedMeshInfos,
                bool apply = true)
            {
                this.offset = offset;
            ...
            public void ShiftVertices(TMP_Text target, Vector3 topLeftShift, Vector3 topRightShift, Vector3 bottomLeftShift,
                Vector3 bottomRightShift)
            {
        ...
        public void SkewSpanX(int fromCharIndex, int toCharIndex, float skewFactor, bool skewTop = true)
        {
            ...
            if (!ValidateSpan(fromCharIndex, toCharIndex, out firstVisibleCharIndex, out lastVisibleCharIndex))
            {
                return;
            }
        ...
        public void ShiftCharVertices(int charIndex, Vector3 topLeftShift, Vector3 topRightShift, Vector3 bottomLeftShift,
            Vector3 bottomRightShift)
        {
            if (!ValidateChar(charIndex))
            {
        ...
        /// <summary>
        ///     Skews the given character horizontally along the X axis and returns the skew amount applied (based on the
        ///     character's size)
        /// </summary>
        /// <param name="charIndex">Character index</param>
        ...
        /// <summary>
        ///     Skews the given character vertically along the Y axis and returns the skew amount applied (based on the character's
        ///     size)
        /// </summary>
        /// <param name="charIndex">Character index</param>
        /// <param name="skewFactor">skew amount</param>
        /// <param name="skewRight">If TRUE skews the right side of the character, otherwise the left one</param>
        /// <param name="fixedSkew">
        ///     If TRUE applies exactly the given <see cref="skewFactor" />,
        ...
        /// <summary>
        ///     Tweens a character's alpha to the given value and returns the <see cref="Tween" />.
        ///     Will return NULL if the <see cref="charIndex" /> is invalid or the character isn't visible.
        /// </summary>
        ...
        public TweenerCore<Color, Color, ColorOptions> DOFadeChar(int charIndex, float endValue, float duration)
        {
            ...
            var t = DOTween.ToAlpha(() => _charTransforms[charIndex].GetColor(textInfo.meshInfo),
                x => { _charTransforms[charIndex].UpdateAlpha(target, x, textInfo.meshInfo); }, endValue, duration);
            return t;
        }

        /// <summary>
        ///     Tweens a character's color to the given value and returns the <see cref="Tween" />.
        ///     Will return NULL if the <see cref="charIndex" /> is invalid or the character isn't visible.
        /// </summary>
        /// <param name="charIndex">The index of the character to tween (will throw an error if it doesn't exist)</param>
        /// <param name="endValue">The end value to reach</param>
        ...
        public TweenerCore<Color, Color, ColorOptions> DOColorChar(int charIndex, Color endValue, float duration)
        {
            ...
            var t = DOTween.To(() => _charTransforms[charIndex].GetColor(textInfo.meshInfo),
                x => { _charTransforms[charIndex].UpdateColor(target, x, textInfo.meshInfo); }, endValue, duration);
            return t;
        }

        /// <summary>
        ///     Tweens a character's offset to the given value and returns the <see cref="Tween" />.
        ///     Will return NULL if the <see cref="charIndex" /> is invalid or the character isn't visible.
        /// </summary>
        /// <param name="charIndex">The index of the character to tween (will throw an error if it doesn't exist)</param>
        /// <param name="endValue">The end value to reach</param>
        ...
        /// <summary>
        ///     Tweens a character's rotation to the given value and returns the <see cref="Tween" />.
        ///     Will return NULL if the <see cref="charIndex" /> is invalid or the character isn't visible.
        /// </summary>
        /// <param name="charIndex">The index of the character to tween (will throw an error if it doesn't exist)</param>
        /// <param name="endValue">The end value to reach</param>
        ...
        public TweenerCore<Quaternion, Vector3, QuaternionOptions> DORotateChar(int charIndex, Vector3 endValue, float duration,
        ...
        /// <summary>
        ///     Tweens a character's scale to the given value and returns the <see cref="Tween" />.
        ///     Will return NULL if the <see cref="charIndex" /> is invalid or the character isn't visible.
        /// </summary>
        /// <param name="charIndex">The index of the character to tween (will throw an error if it doesn't exist)</param>
        /// <param name="endValue">The end value to reach</param>
        ...
        ///     will not
        /// </param>
        public Tweener DOShakeCharOffset(int charIndex, float duration, float strength, int vibrato = 10, float randomness = 90,
            bool fadeOut = true)
        {
        ...
        ///     will not
        /// </param>
        public Tweener DOShakeCharOffset(int charIndex, float duration, Vector3 strength, int vibrato = 10, float randomness = 90,
            bool fadeOut = true)
        {
            if (!ValidateChar(charIndex))
            {
        ...
        ///     will not
        /// </param>
        public Tweener DOShakeCharRotation(int charIndex, float duration, Vector3 strength, int vibrato = 10, float randomness = 90,
            bool fadeOut = true)
        {
            if (!ValidateChar(charIndex))
            {
        ...
        ///     will not
        /// </param>
        public Tweener DOShakeCharScale(int charIndex, float duration, float strength, int vibrato = 10, float randomness = 90,
            bool fadeOut = true)
        {
        ...
        ///     will not
        /// </param>
        public Tweener DOShakeCharScale(int charIndex, float duration, Vector3 strength, int vibrato = 10, float randomness = 90,
            bool fadeOut = true)
        {
            if (!ValidateChar(charIndex))
            {
